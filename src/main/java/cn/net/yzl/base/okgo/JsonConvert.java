    ...
    private T parseParameterizedType(Response response, ParameterizedType type) throws Exception {
        ...
        if (rawType != ResponseData.class) {
            ...
            if (typeArgument == Void.class) {
                ...
                response.close();
                if(response.code() == Constant.NetStatusCode.EXPIRED_LOGIN){
                    ResponseData baseResponse = new ResponseData();
                    baseResponse.setCode(10003);
                    baseResponse.setMessage("登录失效");
                    synchronized (this){
                        //发送消息通知，请求重新登陆
                        MessageEvent event_msg = new MessageEvent();
                        event_msg.setTag(MessageEvent.EventApp.TAG_NOTICE_EXPIRED_LOGIN);
                        EventBus.getDefault().post(event_msg);
                    }
                    return (T)baseResponse;
                }else if(response.code() == 401){
                    ResponseData baseResponse = new ResponseData();
                    baseResponse.setCode(Constant.NetStatusCode.INVALID_TOKEN);
                    return (T)baseResponse;
                }else {
                    ResponseData lzyResponse = Convert.fromJson(jsonReader, type);
                    int code = lzyResponse.code;
                    // 一般来说服务器会和客户端定一个数200表示成功，其余的表示失败，这里根据实际情况修改
                    if (code == 200) {
                        //noinspection unchecked
