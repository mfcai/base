...
public class LogUtils {
    ...
    private static void printDeviceInfo(final String filePath) {
        ...
        int versionCode = 0;
        try {
            PackageInfo pi = AppConfig.getApp()
                    .getPackageManager()
                    .getPackageInfo(AppConfig.getApp().getPackageName(), 0);
            if (pi != null) {
    ...
    public static class Config {
        private Config() {
            ...
            if (Environment.MEDIA_MOUNTED.equals(Environment.getExternalStorageState())
                    && AppConfig.getApp().getExternalCacheDir() != null)
                sDefaultDir = AppConfig.getApp().getExternalCacheDir() + FILE_SEP + "log" + FILE_SEP;
            else {
                sDefaultDir = AppConfig.getApp().getCacheDir() + FILE_SEP + "log" + FILE_SEP;
            }
        }
